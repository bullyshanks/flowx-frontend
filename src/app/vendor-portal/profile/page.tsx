...
import {
  User, Phone, Mail, MapPin, IdCard, Calendar, CheckCircle2, LogOut, Loader2,
} from 'lucide-react';
...
export default function VendorProfilePage() {
  ...
  useEffect(() => {
    Promise.all([authApi.me(), vendorPortalApi.dashboard()])
      .then(([p, s]) => {
        setProfile(p);
        setStats(s);
      })
  ...
  return (
    <>
      ...
      <div className="bg-navy border border-white/[0.08] rounded-2xl overflow-hidden mb-6">
        <div className="bg-gradient-to-br from-flowgreen/20 to-flowgreen/5 p-6 flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-flowgreen to-flowgreen-dark flex items-center justify-center font-syne font-extrabold text-2xl text-white border-4 border-flowgreen/20">
            {profile.name?.[0] || 'V'}
          </div>
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-flowgreen text-xs font-bold uppercase tracking-wide">
                Approved Vendor
              </span>
            </div>
          </div>
        ...
        <div className="p-6 space-y-3.5">
          {[
            ...
            <div
              key={row.label}
              className="flex items-center gap-3.5 py-2.5 border-b border-white/[0.05] last:border-0"
            >
              <div className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0">
                <row.icon size={16} className="text-flowgreen" />
              </div>
      ...
      <div className="bg-navy border border-white/[0.08] rounded-2xl p-6 space-y-3">
        ...
        >
          <LogOut size={14} /> Logout from Vendor Portal
        </button>
      </div>
    </>
  );
